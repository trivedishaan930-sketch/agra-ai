...
export function createRateLimiter(options: { limit: number; windowMs: number; store?: RateLimitStore }) {
  ...
  return {
    async check(key: string): Promise<RateLimitResult> {
      ...
      return { allowed: result.count <= options.limit, limit: options.limit, remaining, resetAt: result.resetAt };
    },
  };
}
