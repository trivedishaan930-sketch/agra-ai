...
export class DefaultExplanationAnalyzer implements ExplanationAnalyzer {
  analyze(
    ...
    const explanation: ExplanationModel = {
      explanationSummary:
        "Truth Intelligence foundation package is explainable and provider-independent.",
      detailedExplanation:
        "This build prepares architecture, contracts, validation, configuration, research hooks, and immutable output models only. No truth scoring formulas, web search, citations, provider calls, or LLM execution were performed.",
      ...
      improvementExplanation: improvement.recommendations
        .map((recommendation) => recommendation.improvementSuggestion)
        .join(" "),
    };
    return {
      explanation,
