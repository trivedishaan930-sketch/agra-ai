...
export class DefaultRiskAnalyzer implements RiskAnalyzer {
  analyze(confidence: ConfidenceAnalysis): RiskAnalysis {
    ...
    const risk: RiskAssessment = {
      ...
      riskSummary:
        "Risk assessment is structural only; no hallucination or factual risk algorithm was executed.",
      riskFactors:
        riskLevel === TruthRiskLevel.High ? ["low_engine_readiness"] : [],
      futureRiskSignals: [
        "hallucination_risk",
        "freshness_risk",
        "source_quality_risk",
    ...
    const warnings: TruthWarning[] =
      riskLevel === TruthRiskLevel.High
        ? [
            {
              code: "HIGH_STRUCTURAL_RISK",
              message: "Input structure is weak for future truth analysis.",
              severity: TruthRiskLevel.High,
              metadata: {},
            },
          ]
        : [];
