...
export class DefaultEvidenceAnalyzer implements EvidenceAnalyzer {
  analyze(
    ...
    const evidenceCoverage = clampScore(
      (structuredSignals / 4) * 0.7 + truth.score * 0.3,
    );
    ...
    const evidence: EvidenceModel = {
      evidenceSummary:
        "Evidence architecture is prepared; no external evidence was fetched or validated.",
      evidenceQuality:
        structuredSignals > 0
          ? resolveTruthLevel(evidenceCoverage)
          : TruthLevel.Unknown,
      evidenceCoverage,
      evidenceStatus:
        structuredSignals === 0
        ...
        ? [
            {
              ...
              message:
                "No supporting engine outputs were provided for future evidence analysis.",
