...
import {
  DefaultTruthLifecycleManager,
  type TruthLifecycleManager,
} from "./lifecycle-manager";
import {
  ImmutableTruthResultAggregator,
  type TruthResultAggregator,
...
export class DefaultTruthEngineOrchestrator implements TruthEngineOrchestrator {
  ...
  constructor(dependencies?: Partial<TruthEngineOrchestratorDependencies>) {
    ...
    this.dependencies = {
      lifecycleManager:
        dependencies?.lifecycleManager ?? new DefaultTruthLifecycleManager(),
      validationCoordinator:
        dependencies?.validationCoordinator ??
        new DefaultValidationCoordinator(),
      pipelineCoordinator:
        dependencies?.pipelineCoordinator ??
        new DefaultPipelineCoordinator(pipeline),
      resultAggregator:
        dependencies?.resultAggregator ?? new ImmutableTruthResultAggregator(),
    };
  }

  createExecutionContext(input: TruthInput): TruthPipelineContext {
    const execution = this.dependencies.lifecycleManager.initialize(
      input.metadata,
    );
    const validatedInput =
      this.dependencies.validationCoordinator.validateInput(input);
    const validatingExecution = this.dependencies.lifecycleManager.transition(
      execution,
      "validating",
    );
    return {
  ...
  analyze(input: TruthInput): TruthIntelligencePackage {
    ...
    const runningContext = {
      ...context,
      execution: this.dependencies.lifecycleManager.transition(
        context.execution,
        "running",
      ),
    };
    const output =
      this.dependencies.pipelineCoordinator.execute(runningContext);
    const validatedOutput =
      this.dependencies.validationCoordinator.validatePackage(output);
    const completedOutput =
      this.dependencies.resultAggregator.aggregate(validatedOutput);
    this.dependencies.lifecycleManager.transition(
      runningContext.execution,
      "completed",
    );
...
export const lifecycleAwareTruthEngine = new DefaultLifecycleAwareTruthEngine(
