import {
  DefaultConfidenceAnalyzer,
  DefaultEvidenceAnalyzer,
  DefaultExplanationAnalyzer,
  DefaultImprovementAnalyzer,
  DefaultReliabilityAnalyzer,
  DefaultRiskAnalyzer,
  DefaultStrengthAnalyzer,
  DefaultTruthAnalyzer,
  DefaultWeaknessAnalyzer,
  type ConfidenceAnalyzer,
  type EvidenceAnalyzer,
  type ExplanationAnalyzer,
  type ImprovementAnalyzer,
  type ReliabilityAnalyzer,
  type RiskAnalyzer,
  type StrengthAnalyzer,
  type TruthAnalyzer,
  type WeaknessAnalyzer,
...
export class TruthPipeline {
  ...
  constructor(dependencies?: Partial<TruthPipelineDependencies>) {
    this.dependencies = {
      ...
      evidenceAnalyzer:
        dependencies?.evidenceAnalyzer ?? new DefaultEvidenceAnalyzer(),
      confidenceAnalyzer:
        dependencies?.confidenceAnalyzer ?? new DefaultConfidenceAnalyzer(),
      riskAnalyzer: dependencies?.riskAnalyzer ?? new DefaultRiskAnalyzer(),
      reliabilityAnalyzer:
        dependencies?.reliabilityAnalyzer ?? new DefaultReliabilityAnalyzer(),
      weaknessAnalyzer:
        dependencies?.weaknessAnalyzer ?? new DefaultWeaknessAnalyzer(),
      strengthAnalyzer:
        dependencies?.strengthAnalyzer ?? new DefaultStrengthAnalyzer(),
      explanationAnalyzer:
        dependencies?.explanationAnalyzer ?? new DefaultExplanationAnalyzer(),
      improvementAnalyzer:
        dependencies?.improvementAnalyzer ?? new DefaultImprovementAnalyzer(),
  ...
  analyze(input: TruthInput): TruthIntelligencePackage {
    ...
    const evidence = this.dependencies.evidenceAnalyzer.analyze(
      normalizedInput,
      truth,
    );
    const confidence = this.dependencies.confidenceAnalyzer.analyze(
      truth,
      evidence,
    );
    const risk = this.dependencies.riskAnalyzer.analyze(confidence);
    const reliability = this.dependencies.reliabilityAnalyzer.analyze(
      truth,
      evidence,
      confidence,
      risk,
    );
    const weakness = this.dependencies.weaknessAnalyzer.analyze(
      normalizedInput,
      risk,
    );
    const strength = this.dependencies.strengthAnalyzer.analyze(
      normalizedInput,
      truth,
      evidence,
    );
    const improvement = this.dependencies.improvementAnalyzer.analyze(weakness);
    const explanation = this.dependencies.explanationAnalyzer.analyze(
      evidence,
      confidence,
      risk,
      weakness,
    ...
    return this.dependencies.validator.validatePackage(
      this.dependencies.packageBuilder.build({
